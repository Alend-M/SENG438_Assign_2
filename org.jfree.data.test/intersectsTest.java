...
public class IntersectsTest {
    ...
    // Test if the range intersects with another range that is entirely below
    @Test
    public void testIntersectionAtUpperBoundary() {
        // Parameter range [10, 15] touches the upper bound of [1,10].
        assertTrue("Expected intersection when parameter range touches the upper boundary",
                testRange.intersects(10.0, 15.0));
    }
    ...
    // Test if the range intersects if the range has no length
    @Test
    public void testZeroLengthRangeInside() {
        // Even though 5.0 lies within [1,10], a zero-length range [5,5] is defined to not intersect.
        assertFalse("Expected no intersection for a zero-length parameter range inside the test range",
                testRange.intersects(5.0, 5.0));
    }
